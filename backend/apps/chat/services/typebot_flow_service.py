...
from apps.chat.services.flow_control import (
    close_conversation_from_bot,
    process_bot_control_instructions,
    transfer_conversation_to_department,
)
...
def close_conversation_from_typebot(conversation: Conversation) -> bool:
    """
    Compat: mantém assinatura pública usada por outras partes do sistema.
    Implementação delegada para o controle universal.
    """
    return close_conversation_from_bot(conversation, source="typebot")


def _execute_transfer_by_department_name(conversation: Conversation, department_name: str) -> bool:
    """Compat: mantém assinatura interna. Implementação delegada para o controle universal."""
    return transfer_conversation_to_department(conversation, department_name, source="typebot")


def _process_instructions_in_texts(conversation: Conversation, texts: List[str]) -> List[str]:
    """Compat: agora delega para o módulo compartilhado."""
    return process_bot_control_instructions(conversation, texts, source="typebot")
